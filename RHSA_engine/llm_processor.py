...
You are a fact extraction system for Linux security patch analysis.
...
This mapping applies to ALL packages (kernel, httpd, glibc, openssl, etc.).
...
You are an impact assessment system for Linux security patches.
...
- Core system libraries involved (glibc, openssl, systemd, etc.) OR
...
You will receive:
- Advisory metadata and technical context
...
1. Executive Summary
   - 2–4 sentences summarizing advisory, impact level, and key actions
2. Advisory Details
   - RHSA ID, title, severity
...
5. Blast Radius & Dependency Considerations
   ...
   - Core system involvement (kernel, glibc, openssl, systemd, etc.)
6. Operational Plan
...
def stage1_extract_facts(data):
    ...
    prompt = (
        ...
        f"⚠️  This applies to ALL packages (kernel, httpd, glibc, openssl, etc.).\n"
        f"\n"
...
def stage2_assess_impact(data, facts):
    ...
    prompt = (
        ...
        f"Affected Packages: {', '.join(data.get('affected_packages', [])[:5])}{'...' if len(data.get('affected_packages', [])) > 5 else ''}\n"
...
def stage3_generate_narrative(data, facts, impact):
    ...
    prompt = (
        f"### ADVISORY CONTEXT\n"
        f"Advisory ID: {data['advisory_id']}\n"
        f"Severity: {data.get('severity', 'Unknown')}\n"
        f"Affected Packages: {', '.join(data.get('affected_packages', []))}\n\n"
